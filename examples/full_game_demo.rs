...
async fn main() {
    ...
    let player = GameObject::new(player_sprite, PlayerData { hp: 100.0, max_hp: 100.0 })
        .with_tag("player")
        .update(|obj, ctx| {
            let speed = 250.0 * ctx.time.deltatime();

            if ctx.input.is_key_down(KeyCode::D){ obj.position.x += speed; }
            if ctx.input.is_key_down(KeyCode::A){ obj.position.x -= speed; }
            if ctx.input.is_key_down(KeyCode::W){ obj.position.y -= speed; }
            if ctx.input.is_key_down(KeyCode::S){ obj.position.y += speed; }
            ...
            if obj.click_ctx(ctx, Side::Left) {
                ...
                println!("Kliknięto w gracza w świecie 2D! HP: {} (ratio: {:.2})", obj.data.hp, hp_ratio);
    ...
    let particle_emitter = Behavior::new(ParticleEmitter::new(), ())
        ...
        .update(|obj, ctx| {
            if ctx.input.is_key_pressed(KeyCode::Space) {
                ...
                obj.inner_mut().emit_burst(m_pos, 30, GOLD, (60.0, 250.0), 7.0, 0.8);
