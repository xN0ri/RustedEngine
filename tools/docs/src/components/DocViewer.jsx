...
const ENGINE_LIFECYCLE_STEPS = [
  {
    ...
    tag: 'Frame Start',
    description: 'Przywrócenie skali UI do domyślnej 1.0 (set_ui_scale(1.0, ZERO)) na początku klatki.',
  },
  {
    title: 'Przejścia Scen & Callbacki on_enter',
    tag: 'Scene System',
    description: 'Wykonanie oczekujących żądań switch_scene, emisja SceneChanged oraz wywołanie on_enter.',
  },
  {
    title: 'Wirtualna Rozdzielczość & Mysz',
    tag: 'Input Remap',
    description: 'Remapowanie wektora myszy w ctx.input z uwzględnieniem offsetu i skali letterboksu.',
  },
  {
    title: 'Aktualizacja Kamery & Wstrząsów',
    tag: 'Camera 2D',
    description: 'Przeliczenie macierzy widoku, tłumienie i losowanie przesunięć shake.',
  },
  {
    title: 'Aktualizacja Logiki Świata',
    tag: 'World Logic Pass',
    description: 'Wywołanie world.update(&mut ctx) dla warstw objects, ui_objects, logic oraz sequences.',
  },
  {
  ...
  },
  {
    ...
    tag: 'Render Pass',
    description: 'Rysowanie świata do render targetu VRT, blitting z letterboxingiem i rysowanie tekstu w natywnej rozdzielczości.',
  },
  {
    title: 'Overlay Kursora & Oczekiwanie',
    tag: 'Frame End',
    description: 'Narysowanie CustomCursor i wywołanie async next_frame().await.',
  },
];
...
const BEHAVIOR_WRAPPER_CARDS = [
  {
    ...
    description: 'Łączy dowolny obiekt rysowalny (Inner: Object), własne dane (Data) oraz opcjonalny update closure.',
    code: 'Behavior::new(sprite, data).update(...)',
  },
  {
    title: 'GameObject<Data>',
    badge: 'Sprite + State',
    description: 'Skrót dla Behavior<Sprite, Data>. Dostęp do pól Sprite przez Deref (position, color, click_ctx).',
    code: 'obj.position.x += 1.0;',
  },
  {
    title: 'LogicObject<Data>',
    badge: 'Invisible Controller',
    description: 'Niewidzialny obiekt logiki wywoływany w klatce bez kosztów renderowania.',
...
const WORLD_LAYERS_CARDS = [
  {
    ...
    description: 'Obiekty renderowane w przestrzeni świata 2D przez kamerę (duszki, tilemapy, cząsteczki).',
    code: 'world.add(sprite)',
  },
  {
    title: 'ui_objects',
    badge: 'Screen Layer',
    description: 'Obiekty UI renderowane w przestrzeni ekranu (panele, przyciski, paski postępu, logi).',
    code: 'world.add_ui(panel)',
  },
  {
    title: 'logic',
    badge: 'Logic Layer',
    description: 'Niewidzialne kontrolery systemowe aktualizowane co klatkę bez kosztu rysowania.',
    code: 'world.add_logic(controller)',
  },
];

export function DocViewer({ doc, allDocs, onNavigateDoc, activeSectionId, onSelectSection }) {
        ...
        <div className="space-y-12">
          {doc.sections.map((section) => (
            <section key={section.id} id={section.id} className="scroll-mt-20">
              <h2 className="text-lg sm:text-xl font-bold text-zinc-100 mb-3 tracking-tight flex items-center gap-2 border-b border-zinc-800/60 pb-2 mt-2">
              ...
              {doc.id === 'getting-started' && section.id === 'engine-lifecycle' && (
                <PipelineFlow steps={ENGINE_LIFECYCLE_STEPS} />
              )}

              {doc.id === 'getting-started' && section.id === 'context-struct' && (
                <ContextFieldsGrid />
              )}

              {doc.id === 'world-objects' && section.id === 'world-structure' && (
                <FeatureGrid items={WORLD_LAYERS_CARDS} />
              )}

              {doc.id === 'world-objects' && section.id === 'behavior-and-wrappers' && (
              ...
              {section.subsections && section.subsections.length > 0 && (
                <div className="mt-8 space-y-8 pl-4 border-l-2 border-zinc-800/60 ml-1">
        ...
        <div className="mt-16 pt-8 border-t border-zinc-800/80 flex items-center justify-between">
          {prevDoc ? (
            <button
              ...
              className="flex items-center gap-3 p-3.5 rounded-xl bg-zinc-950 hover:bg-zinc-900 border border-zinc-800 text-left transition-all cursor-pointer group"
            >
              <ArrowLeft className="w-4 h-4 text-zinc-400 group-hover:-translate-x-1 transition-transform" />
              <div>
                <div className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">Poprzedni</div>
                <div className="text-xs font-bold text-zinc-200">{prevDoc.title}</div>
              </div>
            </button>
          ) : <div />}
          ...
          {nextDoc ? (
            <button
              ...
              className="flex items-center gap-3 p-3.5 rounded-xl bg-zinc-950 hover:bg-zinc-900 border border-zinc-800 text-right transition-all cursor-pointer group"
            >
              <div>
                <div className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">Następny</div>
                <div className="text-xs font-bold text-zinc-200">{nextDoc.title}</div>
              </div>
